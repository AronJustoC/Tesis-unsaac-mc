...
def run(data_file_path):
    ...
    print(f"Estructura cargada: {len(structure.nodes)} nodos y {len(structure.elements)} elementos.")
    ...
    if structure.nodes:
        ...
        if len(first_node_coords) == 3 and any(c != 0 for c in first_node_coords):
            print("Confirmado: La estructura tiene coordenadas 3D y no es plana en el origen.")
        else:
            print("Advertencia: Las coordenadas del primer nodo sugieren una estructura 2D o plana en el origen.")

    # 2. ENSAMBLAR MATRIZ DE RIGIDEZ
    K, _ = assemble_global_matrices(structure) # No necesitamos la matriz de masa para estático
    ...
    if 'static_loads' in input_data and input_data['static_loads']:
        ...
        for load in input_data['static_loads']:
            ...
            if node_id in node_coords:
                ...
                print(f"  - Carga aplicada en el nodo {node_id}: {force_vector}")
            else:
                print(f"Advertencia: El nodo {node_id} definido en 'static_loads' no existe en la estructura.")
    else:
    ...
    if u is not None:
        ...
        scale_factor = input_data.get('analysis_settings', {}).get('post_processing', {}).get('scale_factor', 50)

        plot_deformed_structure(structure, u, scale_factor=scale_factor, mass_node_id=input_data['masses']['node_id'], axial_forces=axial_forces)

if __name__ == "__main__":
    import argparse

    parser = argparse.ArgumentParser(description="Run static analysis workflow.")
