...
def plot_mode_shape(
    ...
    if deformation_scale is None:
        ...
        print(
            f"  - Usando escala de deformación automática: {deformation_scale:.2f}")
    ...
    plotter.add_mesh(original_mesh, color="gray",
                     line_width=1, label="Original")
    plotter.add_mesh(deformed_mesh, color="#dc143c",
                     style='wireframe', line_width=3, label="Modo")
    ...
    if structure.constraints:
        ...
        for node_index, dofs in structure.constraints.items():
            ...
            if num_constrained == 3:
                ...
                symbol = pv.Cone(center=center, direction=[
                                 0, 0, 1], height=symbol_scale * 1.5, radius=symbol_scale, resolution=4)
                plotter.add_mesh(
                    symbol, color="#0073e6", label=label if label not in added_labels else None)
                ...
                cyl_direction = np.array(
                    [1., 0., 0.]) if free_axis_idx != 0 else np.array([0., 1., 0.])
                symbol = pv.Cylinder(center=center, direction=cyl_direction,
                                     radius=symbol_scale*0.7, height=symbol_scale*0.7)
                plotter.add_mesh(
                    symbol, color="#c400c4", label=label if label not in added_labels else None)
                added_labels.add(label)
                ...
                plotter.add_mesh(
                    symbol, color="#00a86b", label=label if label not in added_labels else None)
                added_labels.add(label)
    ...
    plotter.add_text(title, position="upper_edge",
                     color="black", font_size=12, font="arial")
...
def animate_mode_shape(
    ...
    points = np.array([node.coords for node in structure.nodes],
                      dtype=float)  # MODIFICADO: dtype=float
    lines = np.array(
        [
            [2, structure.nodes.index(e.nodes[0]),
             structure.nodes.index(e.nodes[1])]
